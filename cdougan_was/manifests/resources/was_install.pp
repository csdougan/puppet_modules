define cdougan_was::resources::was_install (  
  $imcl                = undef,
  $nexus_path          = undef,
  $nexus_repo          = undef,
  $wasuser             = undef,
  ...
  archive { "${archive_file}":
    ...
    creates         => ["${was_extract_path}/repository.config",
                        "${was_install_path}/properties/version/${was_product_name}.msl"],
    cleanup         => true,
    user            => "${wasuser}",
    group           => "${wasgroup}",
  ...
  exec {"install_was ${was_product_name} ${was_version}":
  	command		      => "${imcl} install ${was_product_name}${was_features} -repositories ${was_extract_path} -installationDirectory ${was_install_path} -acceptLicense -log ${log_dir}/was_${was_version}_main_install.log; chown -R ${wasuser}:${wasgroup} ${was_install_path}",
    path            => '/bin:/usr/bin:/sbin:/usr/sbin',
    creates         => "${was_install_path}/properties/version/${was_product_name}.msl",
